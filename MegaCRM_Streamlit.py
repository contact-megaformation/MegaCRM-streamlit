# MegaCRM_Streamlit.py — CRM + Finance (MB/Bizerte) + InterNotes + Reassign Log + Quick Payment Update
# ====================================================================================================
...
# - Quick update: choose registered client and update same Libellé easily
    ...
    if unread_count == 0:
        ...
        with colB:
            selected_to_read = st.multiselect(
                "اختار رسائل لتعليمها كمقروء", options=unread_df["note_id"].tolist(),
...
if tab_choice == "مداخيل (MB/Bizerte)":
    ...
    # --------- إحصائيات شهرية + أرصدة الصناديق (مهم) ----------
    ...
    with st.expander("عرض التفاصيل"):
        ...
        # --- أرصدة صناديق (تراكمي من جانفي → الشهر المختار) ---
    ...
    # --------- إضافة عملية + دفعة سريعة (نفس الكود السابق) ----------
    # (اختصرنا؛ الجزء هذا نفس اللي كان عندك ويخدم كما هو)
    # ... يمكن تبقي على نفس قسم الإضافة/التحديث السريع اللي عندك (ما بدّلناش فيه)
...
# ============================ تبويب الأدمِن (اختياري: بدون تغييرات كبيرة) ============================
if role == "أدمن":
