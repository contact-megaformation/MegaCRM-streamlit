   # MegaCRM_Streamlit_App.py — Admin + Employees + Dashboard + Search + Edit + Notes + Tags + Reassign + Payments (per-employee & admin locks)
...
def make_client_and_sheet_id():
    try:
        ...
        client = gspread.authorize(creds)
        sheet_id = "1DV0KyDRYHofWR60zdx63a9BWBywTFhLavGAExPIa6LI"  # بدّلها لو يلزم
        return client, sheet_id
...
if role == "أدمن":
    ...
    # ===== Admin Payments (All employees) — Password protected =====
    # --- Admin payments lock helpers ---
    def _admin_pay_session_key_open() -> str: return "admin_payments_ok"
    def _admin_pay_session_key_time() -> str: return "admin_payments_ok_at"
    def _admin_pay_password() -> str:
        try: return str(st.secrets["admin_payments"]["password"])
        except Exception: return "admin123"
    ...
    def admin_payments_lock_ui():
        with st.expander("🔒 حماية «جميع الدفوعات» (Admin)", expanded=not admin_payments_unlocked()):
            if admin_payments_unlocked():
                ...
                with c1: st.success("مفتوح (يغلق آليًا بعد 20 دقيقة).")
                with c2:
...
if role == "موظف" and employee:
    ...
    # عداد: المضافين بلا ملاحظات + فلترة Formation
    ...
    def _session_key_open_for(user_login: str) -> str: return f"payments_ok::{user_login}"
    def _session_key_time_for(user_login: str) -> str: return f"payments_ok_at::{user_login}"
    ...
    def payments_lock_ui(user_login: str):
        with st.expander("🔒 حماية المدفوعات (Password)", expanded=not payments_unlocked(user_login)):
            if payments_unlocked(user_login):
                c1, c2 = st.columns([1,1])
                with c1: st.success("تم فتح قسم المدفوعات (ينتهي بعد 15 دقيقة).")
                with c2:
