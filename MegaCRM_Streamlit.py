# MegaCRM_Streamlit_App.py — CRM + Payments (split Admin/Structure) + Finance(MB/Bizerte)
# - Payments schema: Tel, Formation, Prix, Montant_Admin, Montant_Structure, Date, Echeance, Reste, Branch, Note
# - Reste computed as: Prix - (sum(Admin)+sum(Structure))
# - Alerts when Echeance < today and Reste > 0
# - Admin reports by Branch/Employe
# - Finance (MB/Bizerte) tab unchanged in spirit
...
def fin_read_df(client, sheet_id: str, title: str) -> pd.DataFrame:
    ...
    if not values: return pd.DataFrame(columns=FIN_COLUMNS)
...
def _read_payments_for(sh, phone_norm: str, employee_name: str) -> pd.DataFrame:
    ...
    # recompute Reste using history (show current remainder on last row)
...
def _append_payment(
    ...
    add_now = float(montant_admin) + float(montant_structure)
    reste = max(float(prix_total) - (sum_admin + sum_struct + add_now), 0.0)
...
def load_all_data():
    ...
    for ws in worksheets:
        ...
        if title.endswith("_PAIEMENTS") or title.startswith("_"):  # hide system/payments
            continue
...
if tab_choice == "Finance (MB/Bizerte)":
    ...
    with st.sidebar:
        st.markdown("---"); st.subheader("🔧 إعدادات المالية")
    ...
    # ربط Revenus بعميل مُسجَّل (اقتراح libellé/Employé)
        st.markdown("---")
    ...
    with st.form("fin_add_row_v2"):
        c1, c2, c3 = st.columns(3)
        with c1:
            prix_total = st.number_input("💰 Prix (سعر التكوين)", min_value=0.0, step=10.0)
        with c2:
            montant_admin = st.number_input("🏢 مدفوع الإدارة", min_value=0.0, step=10.0)
        with c3:
            montant_struct = st.number_input("🏫 مدفوع الهيكل", min_value=0.0, step=10.0)

        d1, d2, d3 = st.columns(3)
        with d1:
            date_val = st.date_input("📅 تاريخ الدفع", value=datetime.today())
        with d2:
            echeance = st.date_input("⏰ تاريخ الاستحقاق", value=datetime.today())
        with d3:
            branch = st.selectbox("🏢 الفرع", ["Menzel Bourguiba","Bizerte"])

        libelle  = st.text_input("📝 Libellé", value=client_default_lib)
        employe  = st.text_input("👨‍💼 Employé", value=client_default_emp)
        categorie = st.text_input("📂 Catégorie", value=("Vente" if kind=="Revenus" else "Achat"))
        note_default = f"Client: {selected_client_info['name']} / {selected_client_info['formation']}" if selected_client_info else ""
        note = st.text_area("🗒️ Note", value=note_default)

        total_now = montant_admin + montant_struct
        reste_preview = max(prix_total - total_now, 0.0)
        st.caption(f"💡 المجموع الحالي: {total_now:.2f} — الباقي بعد الحفظ: {reste_preview:.2f}")

        if st.form_submit_button("✅ حفظ العملية"):
            if prix_total <= 0:
                st.error("❌ Prix مطلوب.")
            elif total_now <= 0:
                st.error("❌ لازم على الأقل مدفوع واحد.")
            else:
                fin_append_row(
                    client, SPREADSHEET_ID, fin_title,
                    {
                        ...
                        "Montant_Admin": f"{montant_admin:.2f}",
                        "Montant_Structure": f"{montant_struct:.2f}",
                        "Montant_Total": f"{total_now:.2f}",
                        "Echeance": echeance.strftime("%d/%m/%Y"),
                        "Reste": f"{reste_preview:.2f}",
                        "Branch": branch,
                ...
                st.success("✅ تمت العملية"); st.cache_data.clear(); st.rerun()
    st.stop()
...
if role == "موظف" and employee:
    ...
    def payments_lock_ui(user_login: str):
        with st.expander("🔒 حماية المدفوعات", expanded=not payments_unlocked(user_login)):
            if payments_unlocked(user_login):
                ...
                pwd_try = st.text_input("أدخل كلمة السرّ", type="password", key=f"pwd_{user_login}")
    ...
    if payments_unlocked(employee):
        ...
        if not pay_choices:
            st.info("لا يوجد عملاء للدفوعات.")
        else:
            ...
            if df_payments.empty:
                ...
                paid_admin, paid_struct, prix_max = 0.0, 0.0, 0.0
            else:
                st.dataframe(
                    df_payments[[
            ...
            with st.form("pay_add_form_v2", border=True):
                ...
                with c1:
                    prix_total = st.number_input("💰 سعر التكوين (Prix)", min_value=0.0, step=10.0, value=prix_max, key="v2_prix")
                with c2:
                ...
                if st.form_submit_button("➕ أضف الدفعة"):
                    if prix_total <= 0 or (montant_admin <= 0 and montant_struct <= 0):
                        st.warning("أدخل سعر موجب ومبلغ مدفوع (إدارة أو هيكل).")
                    else:
                        try:
...
# =================== Admin: Read All Payments + Reports ===================
...
def read_all_payments_concat(_spreadsheet_id: str, employees: tuple[str, ...]) -> pd.DataFrame:
    ...
    for emp in employees:
        ...
        df["Date_dt"]     = pd.to_datetime(df["Date"], dayfirst=True, errors="coerce") if "Date" in df.columns else pd.NaT
        df["Echeance_dt"] = pd.to_datetime(df["Echeance"], dayfirst=True, errors="coerce") if "Echeance" in df.columns else pd.NaT
        if "Branch" not in df.columns: df["Branch"] = ""
        all_rows.append(df); time.sleep(0.1)
    if not all_rows:
...
if role == "أدمن":
    ...
    if not admin_unlocked():
        ...
        # إدارة موظفين مختصرة (تبقى نفس اللوجيك متاعك، نجم توسّعها)
        ...
        # تقارير
        st.markdown("---"); st.markdown("### 💳 جميع الدفوعات (كل الموظفين) + تقارير الفروع")
        ...
        if df_allp.empty:
            ...
            with colf4:
                min_d = pd.to_datetime(df_allp["Date_dt"]).min(); max_d = pd.to_datetime(df_allp["Date_dt"]).max()
